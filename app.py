...
.sub-text {
    ...
    margin-bottom: 20px;
        ...
        .insight-box {padding: 16px 18px; border-radius: 14px; background: #111827; border: 1px solid #2563eb; border-left: 6px solid #3b82f6; margin-bottom: 12px; color: #f9fafb !important;}
        .success-box {padding: 16px 18px; border-radius: 14px; background: #052e1a; border: 1px solid #16a34a; border-left: 6px solid #22c55e; margin-bottom: 12px; color: #f9fafb !important;}
        .warning-box {padding: 16px 18px; border-radius: 14px; background: #431407; border: 1px solid #f97316; border-left: 6px solid #fb923c; margin-bottom: 12px; color: #f9fafb !important;}
        .dark-box {padding: 16px 18px; border-radius: 14px; background: #111827; border: 1px solid #334155; color: #ffffff; margin-bottom: 12px;}
    
        .insight-box *, .success-box *, .warning-box *, .dark-box *, .section-card * { color: #f9fafb !important; }
        .kmeans-note {padding: 15px 18px; border-radius: 14px; background: #0f172a; border: 1px solid #475569; color: #f8fafc !important; margin: 10px 0 18px 0; line-height: 1.7;}
        .kmeans-note * {color: #f8fafc !important;}
    </style>
    """,
    unsafe_allow_html=True,
)
...
@st.cache_data
def get_kmeans_k_evaluation(df):
    return evaluate_k_values(df, min_k=2, max_k=8)
...
st.markdown('<div class="sub-text">Separate files version: preprocessing + K-Means + Random Forest +  Streamlit App.</div>', unsafe_allow_html=True)

if filtered.empty:
    st.error("No data available for the selected filters.")
    st.stop()

overview_tab, preprocessing_tab, owner_tab, kmeans_tab, rf_tab, insights_tab = st.tabs(
    [
        "Overview & EDA",
...
with kmeans_tab:
    ...
    <div class="kmeans-note">
        <b>What K-Means does here:</b> It groups customers into business segments based on similar buying behavior.
        It is an unsupervised model, so it does not use accuracy. We evaluate it using clustering metrics.
    </div>
    """, unsafe_allow_html=True)

    st.markdown("### 1. Best Number of Clusters Analysis")
    k_eval_df, suggested_k = get_kmeans_k_evaluation(filtered)

    col_a, col_b, col_c = st.columns(3)
    col_a.metric("Suggested Best K", suggested_k)
    col_b.metric("K Values Tested", f"{int(k_eval_df['K'].min())} - {int(k_eval_df['K'].max())}")
    col_c.metric("Best Silhouette", f"{k_eval_df['Silhouette Score'].max():.3f}")

    st.markdown(f"""
    <div class="success-box">
        <b>Suggested K:</b> {suggested_k}. This recommendation is based on Silhouette Score, Calinski Harabasz Score,
        and Davies Bouldin Score. You can still change K manually using the slider below.
    </div>
    """, unsafe_allow_html=True)

    with st.expander("Show K comparison table"):
        st.dataframe(k_eval_df.round(4), use_container_width=True, hide_index=True)

    col_chart_1, col_chart_2 = st.columns(2)
    with col_chart_1:
        st.plotly_chart(px.line(k_eval_df, x="K", y="Inertia", markers=True, title="Elbow Method: K vs Inertia"), use_container_width=True)
    with col_chart_2:
        st.plotly_chart(px.line(k_eval_df, x="K", y="Silhouette Score", markers=True, title="K vs Silhouette Score"), use_container_width=True)

    st.markdown("### 2. Train Final K-Means Model")
    max_clusters = min(8, max(2, filtered["Customer ID"].nunique() - 1))
    n_clusters = st.slider("Number of Clusters", 2, max_clusters, min(int(suggested_k), max_clusters))

    model_output = get_kmeans_model(filtered, n_clusters)
    if len(model_output) == 7:
        model, scaler, clustered_df, cluster_summary, km_metrics, km_features, recommendations = model_output
    else:
        model, scaler, clustered_df, cluster_summary, km_metrics, km_features = model_output
    ...
    st.markdown("""
    <div class="kmeans-note">
        <b>Metrics meaning:</b><br>
        Silhouette Score: higher is better. It shows how separated the clusters are.<br>
        Inertia: lower is better, but it naturally decreases when K increases.<br>
        Calinski Harabasz: higher is better. It shows better separation between clusters.<br>
        Davies Bouldin: lower is better. It means clusters are less similar to each other.
    </div>
    """, unsafe_allow_html=True)
    ...
    for _, row in cluster_summary.iterrows():
        title = row["Segment Name"] if "Segment Name" in cluster_summary.columns else f"Cluster {row['Cluster']}"
        st.markdown(f"""
        <div class="insight-box">
            <b>{title}</b><br>
            {row['Business Interpretation']}
        </div>
        """, unsafe_allow_html=True)
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.plotly_chart(px.scatter(clustered_df, x="Age", y="Total_Amount", color="Cluster", size="Quantity", hover_data=["Customer ID", "Gender", "Product_Category", "Avg_Order_Value", "Transactions"], title="Customer Segments: Age vs Spending"), use_container_width=True)
    with col2:
    ...
    col3, col4 = st.columns(2)
    with col3:
        st.plotly_chart(px.box(clustered_df, x="Cluster", y="Total_Amount", color="Cluster", title="Spending Distribution by Cluster"), use_container_width=True)
    with col4:
        st.plotly_chart(px.box(clustered_df, x="Cluster", y="Quantity", color="Cluster", title="Quantity Distribution by Cluster"), use_container_width=True)
    ...
    matched_cluster = cluster_summary[cluster_summary["Cluster"] == live_cluster].iloc[0]
    segment_name = matched_cluster["Segment Name"] if "Segment Name" in cluster_summary.columns else f"Cluster {live_cluster}"
    st.success(f"Predicted Cluster: {live_cluster} - {segment_name}")
    st.markdown(f"<div class='success-box'><b>Meaning:</b> {matched_cluster['Business Interpretation']}</div>", unsafe_allow_html=True)

    with st.expander("Show clustered customer data"):
